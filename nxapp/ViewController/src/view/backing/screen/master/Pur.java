...
public class Pur {
    ...
    public void bphase(PhaseEvent phaseEvent) {
        ...
        if(session.getAttribute("purid")==null)
        {                try {
                    Connection con = dbcon.getConnection();
                    Statement stt = con.createStatement();
                    ResultSet rs = stt.executeQuery("select distinct purid from pur");
                    int i = 1;
                    while (rs.next()) {
                        i++;
                    }
                    if (i <= 9) {
                        getIt1().setValue("PUR-0000" + i);
                    }
                    if (i > 9 && i <= 99) {
                        getIt1().setValue("PUR-000" + i);
                    }
                    if (i >= 100 && i <= 999) {
                        getIt1().setValue("PUR-00" + i);
                    }
                    if (i >= 1000 && i <= 9999) {
                        getIt1().setValue("PUR-0" + i);
                    }
                    if (i >= 10000) {
                        getIt1().setValue("PUR-" + i);
                    }
                } catch (Exception er) {
                    er.printStackTrace();
                }}
                ...
                try{
                    ...
                    Statement st=con.createStatement();  
                        ResultSet rs = st.executeQuery("select * from pur where purid = '"+session.getAttribute("purid")+"'");
                    while(rs.next())
                    {
                        ...
                        getId1().setValue(trr);
                            ...
                            getIt6().setValue(rs.getString(5));
                            getSoc1().setValue(rs.getString(7));
                            getSoc2().setValue(rs.getString(8));
                            TableData ex1=new TableData();
                            ex1.setIcode(rs.getString(9));
                            ex1.setIname(rs.getString(10));
                            ex1.setIqty(rs.getString(11));
                            ex1.setIunit(rs.getString(12));
                            ex1.setIprice(rs.getString(13));
                            ex1.setIamount(rs.getString(14));
                            ex1.setItotal(rs.getString("total"));
                            session.removeAttribute("pur_id");
                            adfTable.add(ex1);
    ...
    public void suppValChange(ValueChangeEvent valueChangeEvent) {
        // Add event code here...
        String _val = valueChangeEvent.getNewValue().toString();
        try {
            Connection con = dbcon.getConnection();
            Statement stt = con.createStatement();
            System.out.println("select concat(aline1,' ',aline2,' ',city,' ',state,' ' ,country,' ',pin)),mobile,email from supp where name='" +
                               _val + "'");
            ResultSet rs =
                stt.executeQuery("select concat(aline1,' ',aline2,' ',city,' ',state,' ' ,country,' ',pin),mobile,email from supp where name='" +
                                 _val + "'");
            while (rs.next()) {
                getIt3().setValue(rs.getString(1));

                getIt5().setValue(rs.getString(3));
                getIt6().setValue(rs.getString(2));
            }
    ...
    public void save() {
        ...
        try {
            Connection con = dbcon.getConnection();
            Statement stt = con.createStatement();
            System.out.println(getId1().getValue());
            ...
            for (int i = 0; i < getAdfTable().size()-1; i++) {
                dt = getAdfTable().get(i);
                if (dt.getIname() != null) {
                    stt.execute("insert into pur value('" + getIt1().getValue() + "','" + getIt2().getValue() + "','" +
                                trr + "','" + getIt3().getValue() + "','" + getIt5().getValue() + "','" +
                                getIt6().getValue() + "','" + getSoc1().getValue() + "','" + getSoc2().getValue() +
                                "','" + dt.getIcode() + "','" + dt.getIname() + "','" + dt.getIqty() + "','" +
                                dt.getIunit() + "','" + dt.getIprice() + "','" + dt.getIamount() + "','" +
                                dt.getItax() + "','" + dt.getItotal() + "','"+ 0 +"','"+ 0.00 +"')");

                    stt.execute("insert into vocher values('" + getIt1().getValue() + "','" + trr + "','" + "15" +
                                "','" + "Expenses" + "','" + dt.getIamount() + "','" + 0 + "','" + "1501" + "','" +
                                "Purchase" + "','" + getIt2().getValue() + "','" + dt.getIqty() + "','" +
                                dt.getIprice() + "','" + "0" + "','" + "0" + "','" + "0" + "','" + dt.getIname() +
                                "' )");
                    stt.execute("insert into vocher values('" + getIt1().getValue() + "','" + trr + "','" + "13" +
                                "','" + "Current Laibilities" + "','" + 0 + "','" + dt.getIamount() + "','" + "1301" +
                                "','" + "Sundary Creditors" + "','" + getIt2().getValue() + "','" + dt.getIqty() +
                                "','" + dt.getIprice() + "','" + "0" + "','" + "0" + "','" + "0" + "','" +
                                dt.getIname() + "' )");
                }
               getAdfTable().clear();
    ...
    public void p1_attributeChangeListener(AttributeChangeEvent attributeChangeEvent) {
        // Add event code here...
    }
}
