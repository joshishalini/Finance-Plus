...
public class Prodlist {
    ...
    public void bphase(PhaseEvent phaseEvent) {
        ...
        try {
            if (AdfFacesContext.getCurrentInstance().isPartialRequest(phaseEvent.getFacesContext())) {
                ...
                ResultSet rs =
                    stt.executeQuery("select idproduct,name,price,unit,cate from product group by idproduct");
                while (rs.next()) {
    ...
    public void choicechange(ValueChangeEvent valueChangeEvent) {
        ...
        if (getSoc1().getValue().equals("By Name")) {
              AdfFacesContext.getCurrentInstance().addPartialTarget(it1);
            getIt1().setDisabled(false);}
        if (getSoc1().getValue().equals("By Price")) {
              AdfFacesContext.getCurrentInstance().addPartialTarget(it1);
            getIt1().setDisabled(false);}
    }
    ...
    public void valchange(ValueChangeEvent valueChangeEvent) {
        ...
        if (getSoc1().getValue().toString().equalsIgnoreCase("By name")) {
            try {
                ...
                Statement stt = con.createStatement();
                ResultSet rs =
                    stt.executeQuery("select idproduct,name,price,unit,cate from product where name like" +
                                     "'%" + valueChangeEvent.getNewValue().toString() + "%'" + " group by idproduct");
                getAdfTable().clear();
                while (rs.next()) {
                    TableData ex1 = new TableData();
                    ex1.setIunit(rs.getString(1));
        ...
        if (getSoc1().getValue().equals("By Price")) {
            try {
                ...
                Statement stt = con.createStatement();
                ResultSet rs =
                    stt.executeQuery("select idproduct,name,price,unit,cate from product where price like" +
                                     "'%" + valueChangeEvent.getNewValue().toString() + "%'" + " group by idproduct");
                getAdfTable().clear();
                while (rs.next()) {
                    TableData ex1 = new TableData();
        ...
        if (getSoc1().getValue().equals("By Id")) {
            try {
                ...
                Statement stt = con.createStatement();
                ResultSet rs =
                    stt.executeQuery("select idproduct,name,price,unit,cate from product where idproduct like" +
                                     "'%" + valueChangeEvent.getNewValue().toString() + "%'" + " group by idproduct");
                getAdfTable().clear();
                while (rs.next()) {
                    TableData ex1 = new TableData();
    ...
    public String edit() {
        ...
        String idproduct = getAdfTable().get(getT1().getRowIndex()).getIunit();
        session.putValue("idproduct", idproduct);
        return "product.jsf";
    }
