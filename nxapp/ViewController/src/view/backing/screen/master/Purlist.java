...
public class Purlist {
    ...
    public void bphase(PhaseEvent phaseEvent) {
        ...
        try {
            if (_update_flg == 1) {
                ...
                ResultSet rs =
                    stt.executeQuery("select purid,supp_name,sum(amount),status,term,sum(amount)-amtpaid from pur group by purid");
                while (rs.next()) {
    ...
    public void ValChange(ValueChangeEvent valueChangeEvent) {
        if (getSoc2().getValue().equals("By name")) {
            try {
                ...
                Statement stt = con.createStatement();
                ResultSet rs =
                    stt.executeQuery("select purid,supp_name,sum(amount),status,term,sum(amount)-amtpaid from pur where supp_name like" +
                                     "'%" + valueChangeEvent.getNewValue().toString() + "%'" + " group by purid");
                getAdfTable().clear();
                while (rs.next()) {
                    TableData ex1 = new TableData();
                    ex1.setIunit(rs.getString(1));
        ...
        if (getSoc2().getValue().equals("By id")) {
            try {
                ...
                Statement stt = con.createStatement();
                ResultSet rs =
                    stt.executeQuery("select purid,supp_name,sum(amount),status,term,sum(amount)-amtpaid from pur where purid like" +
                                     "'%" + valueChangeEvent.getNewValue().toString() + "%'" + " group by purid");
                getAdfTable().clear();
                while (rs.next()) {
                    TableData ex1 = new TableData();
    ...
    public void choiceChange(ValueChangeEvent valueChangeEvent) {
        ...
        if (getSoc2().getValue().equals("Paid")) {
            try {
                ...
                Statement stt = con.createStatement();
                ResultSet rs =
                    stt.executeQuery("select purid,supp_name,sum(amount),status,term,sum(amount)-amtpaid from pur where status ='" +
                                     1 + "' group by purid");
                getAdfTable().clear();
                while (rs.next()) {
                    TableData ex1 = new TableData();
    ...
    public void selectionlist(SelectionEvent selectionEvent) {
        ...
        String purid = getAdfTable().get(getT1().getRowIndex()).getIunit();
        session.putValue("purid", purid);
    ...
    public void save() {
        ...
        try {
            ...
            if (_paid_amount > _due_amount) {
                ...
                Connection con = dbcon.getConnection();
                Statement stt = con.createStatement();
                System.out.println("update pur set amtpaid= amtpaid + " + getIt6().getValue() + " where purid=" +
                                   getIt5().getValue());
                stt.execute("update pur set amtpaid= amtpaid + " + getIt6().getValue() + " where purid='" +
                            getIt5().getValue() + "'");
                if (_paid_amount == _due_amount) {
                    stt.execute("update pur set status=1 where purid='" + getIt5().getValue() + "'");
                    //creating vocherr
                ...
                stt.execute("insert into vocher values('" + getIt5().getValue() + "','" + trr + "','" + "13" + "','" +
                            "Current Laibilities" + "','" + getIt6().getValue() + "','" + 0 + "','" + "1301" + "','" +
    ...
    public String newpur() {
        // Add event code here...
        return "pur.jsf";
    }

    public String bt1_action() {
        // Add event code here...
        return null;
    }
}
