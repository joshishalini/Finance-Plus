...
public class Details {
    ...
    public void setInvoice_table(List<TableData> invoice_table) {
        Details.invoice_table = invoice_table;
    }
    ...
    public void setTransaction_table(List<TableData> transaction_table) {
        Details.transaction_table = transaction_table;
    }
        ...
        if (AdfFacesContext.getCurrentInstance().isPartialRequest(phaseEvent.getFacesContext())) {
            ...
            if (session.getAttribute("idcust") == null) {
                ...
                try {
                    ...
                    System.out.println("select idcust,name,concat(aline1,' 'aline2,' 'city,' 'pin,' 'state,' 'country),sum(amount)-amtpaid from cust where idcust = '" +
                                     session.getAttribute("idcust") + "'");
                    ResultSet rs =
                        st.executeQuery("select idcust,name,concat(aline1,' ',aline2,' ',city,' ',state,' ' ,country,' ',pin) from cust where idcust = '" +
                                        session.getAttribute("idcust") + "'");
                    while (rs.next()) {
            ...
            try {
                {
                    ...
                    System.out.println("select cust_name,invoiceid,total,amtpaid from invoice where cust_name ='" +
                                       getOl6().getValue() + "'");
                    ResultSet rs =
                        stt.executeQuery("select invoiceid,total,sum(amount)-amtpaid from invoice where cust_name ='" +
                                         getOl6().getValue() + "' group by invoiceid");
                    int i = 1;
                    while (rs.next()) {
            ...
            try {
                {
                    ...
                    System.out.println("select perticulers,dr,cr,rate from vocher where cname ='" +
                                          getOl6().getValue() + "'");
                    ResultSet rss =
                        sttt.executeQuery("select perticulers,dr,cr,rate from vocher where cname ='" +
                                          getOl6().getValue() + "'");
                    int i = 1;
                    double bal = 0;
                    while (rss.next()) {
