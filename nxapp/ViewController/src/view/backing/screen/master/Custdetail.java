...
public class Custdetail {
    ...
    public void bphase(PhaseEvent phaseEvent) {
        ...
        try {
            if (AdfFacesContext.getCurrentInstance().isPartialRequest(phaseEvent.getFacesContext())) {
                ...
                ResultSet rs =
                    stt.executeQuery("select idcust,name,email,phno,concat(aline1,' ',aline2,' ',city,' ',state,' ' ,country,' ',pin) from cust group by idcust");
                while (rs.next()) {
    ...
    public void valchange(ValueChangeEvent valueChangeEvent) {
        ...
        if (getSoc1().getValue().toString().equalsIgnoreCase("By name")) {
            try {
                ...
                Statement stt = con.createStatement();
                ResultSet rs =
                    stt.executeQuery("select idcust,name,email,phno,concat(aline1,' ',aline2,' ',city,' ',state,' ' ,country,' ',pin) from cust where name like" +
                                     "'%" + valueChangeEvent.getNewValue().toString() + "%'" + " group by idcust");
                getAdfTable().clear();
                while (rs.next()) {
                    TableData ex1 = new TableData();
                    ex1.setIunit(rs.getString(1));
        ...
        if (getSoc1().getValue().equals("By Mobile NO")) {
            try {
                ...
                Statement stt = con.createStatement();
                ResultSet rs =
                    stt.executeQuery("select idcust,name,email,phno,concat(aline1,' ',aline2,' ',city,' ',state,' ' ,country,' ',pin) from cust where phno like" +
                                     "'%" + valueChangeEvent.getNewValue().toString() + "%'" + " group by idcust");
                getAdfTable().clear();
                while (rs.next()) {
                    TableData ex1 = new TableData();
        ...
        if (getSoc1().getValue().equals("By Id")) {
            try {
                ...
                Statement stt = con.createStatement();
                ResultSet rs =
                    stt.executeQuery("select idcust,name,email,phno,concat(aline1,' ',aline2,' ',city,' ',state,' ' ,country,' ',pin) from cust where idcust like" +
                                     "'%" + valueChangeEvent.getNewValue().toString() + "%'" + " group by idcust");
                getAdfTable().clear();
                while (rs.next()) {
                    TableData ex1 = new TableData();
    ...
    public String edit() {
        ...
        String idcust = getAdfTable().get(getT1().getRowIndex()).getIunit();
        session.putValue("idcust", idcust);
        return "cust.jsf";
    }
